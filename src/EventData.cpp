...
EventData::EventData() : camera_resolution(0.0f), diffScale(0.0f),
                         ...
                         evtParticlesSSBO(0), outputDataSSBO(0), countersSSBO(0), computeInitialized(false) {}
...
void EventData::initParticlesFromFile(const std::string &filename)
{
    ...
    while (reader.isRunning())
    {
        if (const auto events = reader.getNextEventBatch(); events.has_value())
        {
            ...
            {
                if (counter++ % modFreq != 0)
                {
                    continue;
                } // TODO instead skip batch if possible
...
void EventData::initComputeShader()
{
    ...
    computeProg.setShaderName(resourceDir + "event_processing.comp");
    if (!computeProg.init())
    {
