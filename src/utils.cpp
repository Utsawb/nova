...
void initLabels()
{
    unitLabels = vector<string>({"Time Window [%.3f, %.3f] (",
                                 "Frame Period (",
                                 "Shutter Initial (",
                                 "Shutter Final (",
             ...
             std::string &video_name, bool &recording, std::string &datadirectory, bool &loadFile)
{
    ...
    if (dTimeWindow)
    { // Ensure windows match (time window == events window)
    ...
    if (dTimeWindow || dEventWindow || dProcessingOptions)
    { // Ensure internal shutter values match for both options
        if (evtData->getShutterType() == EventData::TIME_SHUTTER)
        {
