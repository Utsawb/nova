...
void ComputeProgram::setShaderName(const std::string &c)
{
    cShaderName = c;
}
...
bool ComputeProgram::init()
{
    ...
    if (!cFile.is_open())
    {
        std::cerr << "Failed to open compute shader file: " << cShaderName << std::endl;
        return false;
    }
    ...
    if (verbose)
    {
        std::cout << "Compute shader compiled successfully: " << cShaderName << std::endl;
    }
...
void ComputeProgram::dispatch(GLuint numGroupsX, GLuint numGroupsY, GLuint numGroupsZ)
{
